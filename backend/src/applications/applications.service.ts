...
export class ApplicationsService {
  ...
  async listForEvent(
    eventId: string,
    opts?: { page?: number; pageSize?: number; status?: ApplicationStatus | 'all' }
  ): Promise<{ items: ApplicationEntity[]; total: number; page: number; pageSize: number }> {
    const page = Math.max(1, Math.floor(opts?.page ?? 1));
    const pageSize = Math.min(100, Math.max(1, Math.floor(opts?.pageSize ?? 10)));
    const where: any = { eventId };
    if (opts?.status && opts.status !== 'all') where.status = opts.status;
    const [items, total] = await this.repo.findAndCount({ where, skip: (page - 1) * pageSize, take: pageSize, order: { createdAt: 'DESC' } });
    return { items, total, page, pageSize };
  }

  async submit(dto: CreateApplicationDto & { inviteCode?: string }): Promise<ApplicationEntity> {
    ...
    if (form) {
      try {
        ...
        const fields: Array<{ name: string; required?: boolean }> = Array.isArray(schema?.fields)
          ? schema.fields
          : [];
        const requiredNames = fields.filter((f) => f?.required).map((f) => f.name).filter(Boolean);
  ...
  async setFormSchema(eventId: string, schema: unknown): Promise<ApplicationFormEntity> {
    ...
    const created = this.forms.create({ eventId, schema: JSON.stringify(schema ?? {}) });
    return await this.forms.save(created);
  }

  async getFormSchema(eventId: string): Promise<{ eventId: string; schema: unknown } | null> {
